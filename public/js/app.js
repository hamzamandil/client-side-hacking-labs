...
const LABS = [
  ...
  // --- PostMessage XSS ---
  { id: 'postmsg-1', category: 'postmessage', title: 'No Origin Check', description: 'A widget listens for postMessage events but never checks the origin. Send a malicious message from your attacker page to trigger XSS.', difficulty: 'easy', tags: ['postMessage', 'no validation'], url: '/labs/postmessage/level1.html', writeup: '/writeups/postmessage-no-origin-check', writeupAuthor: 'In-house writeup' },
  ...
  { id: 'cspt-1', category: 'cspt', title: 'Basic Path Injection', description: 'A profile page fetches /api/notifications/{userId}. The userId comes from a URL parameter. Traverse the path to access other API endpoints.', difficulty: 'easy', tags: ['CSPT', 'fetch', 'path traversal'], url: '/labs/cspt/level1.html', writeup: '/writeups/cspt-to-xss', writeupAuthor: 'In-house writeup' },
  ...
  { id: 'chain-4', category: 'chains', title: 'The Grand Chain: CSPT + PostMessage + OAuth = ATO', description: 'The ultimate challenge. A CSPT vulnerability lets you inject a response that triggers a postMessage to the parent. The parent uses this to initiate an OAuth flow with a manipulated redirect. Chain all three to steal the token and take over the account.', difficulty: 'expert', tags: ['CSPT', 'postMessage', 'OAuth', 'ATO', 'mega chain'], url: '/labs/chains/level4.html', writeup: '/writeups/mega-chain-ato', writeupAuthor: 'In-house writeup' },
  { id: 'chain-uber-selfxss', category: 'chains', title: 'RideMetric: Self-XSS \u2192 Login CSRF \u2192 ATO', description: 'A driver partner portal has stored XSS in the bio field, but you can only edit your own profile. The SSO callback has no state parameter (login CSRF). Logout is GET-based. Chain them: log victim out, silently log them into the attacker account, trigger XSS in their browser. Classic Whitton/Uber pattern.', difficulty: 'expert', tags: ['self-XSS', 'login CSRF', 'SSO', 'ATO', 'Whitton'], url: '/labs/chains/uber-self-xss.html', writeup: 'https://whitton.io/articles/uber-turning-self-xss-into-good-xss/', writeupAuthor: 'Jack Whitton (Uber)' },
  { id: 'chain-dirty-dancing', category: 'chains', title: 'Flowpay: OAuth Dirty Dancing + PostMessage Href Leak', description: 'A business payments platform accepts OAuth implicit flow. The callback page has an analytics postMessage handler that responds to any caller with location.href \u2014 including the access_token fragment. Open the flow in a popup, query the gadget, steal the token.', difficulty: 'expert', tags: ['OAuth', 'implicit flow', 'postMessage', 'href leak', 'dirty dancing'], url: '/labs/chains/dirty-dancing-postmessage.html', writeup: '/writeups/oauth-dirty-dancing', writeupAuthor: 'In-house writeup' },
  { id: 'chain-dd-multi', category: 'chains', title: 'Dash: Multi-Origin OAuth Dirty Dancing (window.name leak)', description: 'Three origins (main :3000, IdP :4000, sandbox :5000). Implicit flow keeps the token in the URL even on invalid state. Main app stores location into iframe.name before loading the sandbox. Sandbox has XSS via ?src=. Chain: malicious telemetry JS runs in sandbox, reads window.name, exfils token.', difficulty: 'expert', tags: ['OAuth', 'multi-origin', 'window.name', 'iframe XSS', 'dirty dancing'], url: '/labs/chains/dd-multi-origin.html', writeup: '/writeups/dd-multi-origin', writeupAuthor: 'In-house writeup' },
  ...
  { id: 'rw-window-hijack', category: 'realworld', title: 'WorkHub: window.open Target Hijacking', description: 'A SaaS addon marketplace uses a predictable window.open target name for OAuth popups. Hijack the browsing context to force-link a malicious addon to the victim\'s workspace.', difficulty: 'hard', tags: ['window.open', 'OAuth', 'popup hijack', 'CTBB'], url: '/labs/realworld/window-open-hijack.html', writeup: 'https://lab.ctbb.show/research/can-a-predicted-window-open-target-really-be-that-impactful', writeupAuthor: 'CTBB Lab' },
  { id: 'rw-cspt-delete', category: 'realworld', title: 'SecureVault: CSPT in DELETE Requests', description: 'A session management page builds DELETE fetch URLs from user input. Traverse the path to hit destructive endpoints — delete accounts, initiate transfers. Read the JS, find the source, exploit the sink.', difficulty: 'hard', tags: ['CSPT', 'DELETE', 'path traversal', 'Deepstrike'], url: '/labs/realworld/cspt-delete.html', writeup: 'https://deepstrike.io/blog/breaking-client-side-security-path-traversal-in-delete-requests', writeupAuthor: 'Deepstrike' },
  { id: 'rw-postmsg-iframe', category: 'realworld', title: 'CloudDocs: PostMessage Iframe Injection', description: 'A document platform creates floating widget panels from postMessage data. The handler builds iframes via string concatenation with no origin check and no sanitization. Break out of the attribute to achieve XSS.', difficulty: 'hard', tags: ['postMessage', 'iframe injection', 'attribute breakout'], url: '/labs/realworld/postmessage-iframe-xss.html', writeup: 'https://medium.com/@youghourtaghannei/postmessage-xss-vulnerability-on-private-program-18e773e1a1ba', writeupAuthor: 'Youghourta Ghannei' },
  ...
  // --- elmahdi4 writeup labs (with hidden hint panels) ---
  { id: 'elmahdi-cookie-xss', category: 'advanced-xss', title: 'Accelr: Cookie Overwrite Bypass', description: 'The /login endpoint overwrites the lang cookie server-side, blocking cookie-based XSS. But another endpoint uses the same vulnerable JS without the protection. Find the unpatched path and exploit.', difficulty: 'hard', tags: ['cookie', 'Set-Cookie bypass', 'DOM XSS', 'endpoint inconsistency'], url: '/elmahdi/login', writeup: 'https://elmahdi4.wordpress.com/2025/09/26/dom-xss-bypassing-server-side-cookie-overwrite-chrome-innerhtml-quirk-and-json-injection/', writeupAuthor: 'elmahdi4' },
  { id: 'elmahdi-chrome-quirk', category: 'advanced-xss', title: 'PayFlow: Chrome innerHTML Quirk', description: 'A checkout page "safely" decodes HTML-encoded postMessage data using a detached <p> element. The developer thought this was safe. Chrome disagrees — image onerror handlers fire even on detached elements.', difficulty: 'expert', tags: ['postMessage', 'Chrome quirk', 'detached element', 'innerHTML'], url: '/elmahdi/checkout', writeup: 'https://elmahdi4.wordpress.com/2025/09/26/dom-xss-bypassing-server-side-cookie-overwrite-chrome-innerhtml-quirk-and-json-injection/', writeupAuthor: 'elmahdi4' },
  { id: 'elmahdi-json-injection', category: 'advanced-xss', title: 'Fluent: JSON Injection via Querystring', description: 'A config endpoint reflects the version param into its JSON response without escaping. The client merges a specific config field into the window object. Break the JSON structure, inject a window property, pop a javascript: URI.', difficulty: 'expert', tags: ['JSON injection', 'window pollution', 'querystring reflection', 'Object.assign'], url: '/elmahdi/app', writeup: 'https://elmahdi4.wordpress.com/2025/09/26/dom-xss-bypassing-server-side-cookie-overwrite-chrome-innerhtml-quirk-and-json-injection/', writeupAuthor: 'elmahdi4' },

  // --- Advanced PostMessage Labs (jub0bs chain, Microsoft MSRC, Intigriti CTF) ---
  { id: 'pm-adv-dispatch', category: 'pm-advanced', title: 'SearchHub: Method Dispatch via postMessage', description: 'A minified message handler walks dot-notation property paths on window and calls the result. Combined with a JSONP endpoint that double-decodes callbacks, this becomes XSS. Based on jub0bs\' real-world 5-bug chain.', difficulty: 'expert', tags: ['postMessage', 'method dispatch', 'JSONP', 'double-decode', 'jub0bs'], url: '/labs/postmsg-advanced/method-dispatch.html', writeup: 'https://jub0bs.com/posts/2023-05-05-smorgasbord-of-a-bug-chain/', writeupAuthor: 'jub0bs' },
  { id: 'pm-adv-token', category: 'pm-advanced', title: 'AzureBoard: Token Forwarding via Wildcard', description: 'A project dashboard sends auth tokens to extension iframes using postMessage with targetOrigin: "*". The source check has a logical flaw. Any window can request and receive the token. Based on Microsoft MSRC research.', difficulty: 'hard', tags: ['postMessage', 'token theft', 'wildcard origin', 'Microsoft MSRC'], url: '/labs/postmsg-advanced/token-forward.html', writeup: 'https://www.microsoft.com/en-us/msrc/blog/2025/08/postmessaged-and-compromised', writeupAuthor: 'Microsoft MSRC' },
  { id: 'pm-adv-regex', category: 'pm-advanced', title: 'TeamSync: Regex Origin Bypass', description: 'Integration handler validates origin with a regex that has TWO classic bugs: unescaped dot and missing $ anchor. Bypass it from an attacker domain to steal user data and API keys. Based on Microsoft MSRC research.', difficulty: 'hard', tags: ['postMessage', 'regex bypass', 'unescaped dot', 'missing anchor', 'data theft'], url: '/labs/postmsg-advanced/regex-bypass.html', writeup: 'https://www.microsoft.com/en-us/msrc/blog/2025/08/postmessaged-and-compromised', writeupAuthor: 'Microsoft MSRC' },
  { id: 'pm-adv-encoding', category: 'pm-advanced', title: 'Analytics Pro: JS Escape + Filter Bypass', description: 'A ref parameter is reflected inside a JS string in a script tag. The server strips < > " \' = to prevent XSS. Bypass the filter using JS hex escape sequences (\\x22) that survive the filter but parse as quotes in the JS engine.', difficulty: 'expert', tags: ['filter bypass', 'hex escape', 'JS context', 'script reflection', 'Intigriti CTF'], url: '/labs/postmsg-advanced/double-encoding.html', writeup: 'https://www.intigriti.com/researchers/blog/hacking-tools/december-ctf-challenge-xs-leaks-postmessage-xss', writeupAuthor: 'Intigriti' },

  // --- Cache Attacks (cache deception, CSPT+cache chain, cache poisoning) ---
  { id: 'cache-cspt-deception', category: 'cache', title: 'Nimbus: CSPT + Cache Deception = ATO', description: 'The /v1/token endpoint returns your JWT but isn\'t cacheable. The user-lookup endpoint has CSPT. Chain them — browser normalizes path traversal, token response gets cached at a .css URL, attacker reads it without auth. Zero-interaction ATO.', difficulty: 'expert', tags: ['cache deception', 'CSPT', 'path traversal', 'ATO', 'zere.es'], url: '/labs/cache/cspt-cache-deception.html', writeup: 'https://zere.es/posts/cache-deception-cspt-account-takeover/', writeupAuthor: 'zere.es' },
  { id: 'cache-basic-deception', category: 'cache', title: 'Loop: Classic Web Cache Deception', description: 'The account page returns sensitive data (API keys, session tokens, billing). The CDN caches URLs by extension. The origin returns account data for any path under /account. Trick the CDN into caching a logged-in response at a static URL.', difficulty: 'hard', tags: ['cache deception', 'extension', 'Omer Gil pattern', 'static URL'], url: '/labs/cache/basic-cache-deception.html', writeup: '/writeups/cache-deception-cspt-chain', writeupAuthor: 'In-house writeup' },
  { id: 'cache-poison-xfh', category: 'cache', title: 'NewsWire: Cache Poisoning (X-Forwarded-Host)', description: 'The homepage builds its analytics script URL from X-Forwarded-Host. The CDN cache key doesn\'t include this header. Send one request with a poisoned header — every subsequent visitor loads your JS. Classic unkeyed header poisoning.', difficulty: 'hard', tags: ['cache poisoning', 'X-Forwarded-Host', 'unkeyed header', 'PortSwigger'], url: '/labs/cache/cache-poisoning-xfh.html', writeup: 'https://portswigger.net/research/practical-web-cache-poisoning', writeupAuthor: 'PortSwigger' }
];
...
const CATEGORIES = {
  ...
  'advanced-xss': { name: 'Advanced XSS (elmahdi4)', icon: '&#129504;', description: 'Browser quirks, inconsistent server-side defenses, and structural injection. Three realistic apps with hidden hint panels — click "Stuck?" only if you\'re truly blocked. Cookie overwrite bypass, Chrome detached innerHTML quirk, JSON structure injection.' },
  'pm-advanced': { name: 'PostMessage: Advanced', icon: '&#128272;', description: 'Real-world postMessage exploitation beyond "no origin check." Method dispatch chains, token forwarding via wildcard, regex origin bypass, and JS escape filter bypass. Based on jub0bs, Microsoft MSRC, and Intigriti CTF research.' },
  'cache': { name: 'Cache Attacks', icon: '&#128230;', description: 'Web cache deception, cache poisoning, and CSPT + cache chains. Force auth responses to be cached at static URLs. Poison caches via unkeyed headers. One-click account takeover via browser URL normalization + CDN behavior. Based on zere.es, Omer Gil, PortSwigger research.' }
};
...
function renderLabCard(lab, num) {
  ...
  const hasWriteup = !!lab.writeup;
  const isExternalWriteup = hasWriteup && lab.writeup.startsWith('http');
  const writeupIcon = isExternalWriteup ? '&#x2197;' : '&#x1f4d6;';
  const writeupBtn = hasWriteup
    ? `<button class="writeup-btn" onclick="event.stopPropagation(); window.open('${lab.writeup}', '${isExternalWriteup ? '_blank' : '_self'}')" title="Read writeup: ${lab.writeupAuthor || ''}">${writeupIcon} Writeup${lab.writeupAuthor ? ' &middot; ' + lab.writeupAuthor : ''}</button>`
    : '';
  return `
    <div class="lab-card ${solved ? 'solved' : ''} ${isCapstone ? 'capstone-card' : ''} ${hasWriteup ? 'has-writeup' : ''}" onclick="window.location.href='${lab.url}'">
